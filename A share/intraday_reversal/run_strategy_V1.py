...
# -*- coding: utf-8 -*-
"""
V1 (T 15:00 → T+1 14:56) — Embedded ZIP path, CSV outputs
"""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs_V1")
TOPK = 5
COST_PER_SIDE = 0.0003
SIG_START = time(14,26)  # signal 14:26→14:56
SIG_END   = time(14,56)
ENTRY_T   = time(15,0)   # enter at T 15:00
...
def run():
    ...
    try:
        ...
        def make_metrics(sr_obj, bh_obj):
            ...
            rows=[]
            for label,s,tag in [("V1 (T 15:00 → T+1 14:56)",str_tr,"Train"),("V1 (T 15:00 → T+1 14:56)",str_te,"Test"),("V1 (T 15:00 → T+1 14:56)",str_fu,"Full"),
        ...
        plt.figure(figsize=(9,4.2)); plt.plot(eq_str.index, eq_str.values, label="V1 (T 15:00 → T+1 14:56)")
        plt.plot(eq_bh.index, eq_bh.values, label="Buy&Hold (EW, same window)")
        plt.title("Equity Curve — V1 (T 15:00 → T+1 14:56)"); plt.xlabel("Date"); plt.ylabel("Equity (Start=1)"); plt.legend(); plt.tight_layout()
        ...
        plt.hist(trades_per_day.values, bins=bins); plt.title("Trading Frequency — V1")
