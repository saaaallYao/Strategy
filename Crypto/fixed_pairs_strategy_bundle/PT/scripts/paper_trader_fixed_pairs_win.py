...
# Ensure both repo root and code dir are on sys.path (robust across Windows/POSIX separators).
for path in (PROJECT_ROOT, CODE_DIR):
    s = str(path)
    if s not in sys.path:
        sys.path.insert(0, s)

from live_trading_fixed_pairs import LiveFixedPairsTrader, configure_logging  # noqa: E402
