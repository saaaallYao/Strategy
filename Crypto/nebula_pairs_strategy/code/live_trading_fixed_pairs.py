...
def get_log_paths(prefix: str = "nebula_pairs") -> tuple[Path, Path, Path, Path]:
    ...
    signal_state_log = base_dir / f"paper_signal_state_{prefix}_1.csv"
    return trade_log, equity_log, signal_log, signal_state_log
...
class LiveFixedPairsTrader:
    ...
    output_prefix: str = "nebula_pairs"
    ...
    signal_state_log_path: Optional[Path] = None
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.trade_log_path is None
            or self.equity_log_path is None
            or self.signal_log_path is None
            or self.signal_state_log_path is None
        ):
            trade_log, equity_log, signal_log, signal_state_log = get_log_paths(self.output_prefix)
            ...
            self.signal_state_log_path = self.signal_state_log_path or signal_state_log
        ...
        self.signal_state_log_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _log_signal_state(
        ...
        header_needed = not self.signal_state_log_path.exists()
        weight_cols = list(weights.index)
        z_cols = list(zscores.index)
        header_z_cols = [f"{col}_z" for col in z_cols]
        price_cols = list(prices.index)
        with self.signal_state_log_path.open("a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            if header_needed:
                writer.writerow(["timestamp", *weight_cols, *header_z_cols, "leverage", *[f"{col}_px" for col in price_cols]])
            writer.writerow(
                [
                    timestamp,
                    *[f"{weights.get(sym, 0.0):.6f}" for sym in weight_cols],
                    *[f"{zscores.get(sym, 0.0):.6f}" for sym in z_cols],
                    f"{leverage:.6f}",
                    *[f"{prices.get(sym, 0.0):.6f}" for sym in price_cols],
                ]
            )
    ...
    def _rebalance_once(self) -> int:
        ...
        try:
            weights, state = self.engine.latest_weights(self.history)
        ...
        zscores = state.zscores.iloc[-1]
        leverage = float(state.results["leverage"].iloc[-1])
        self._log_signal(ts, weights)
        self._log_signal_state(ts, weights, zscores, leverage, prices)
        self._log_equity(ts)
...
def create_trader_from_env() -> LiveFixedPairsTrader:
    ...
    prefix = os.environ.get("FP_LOG_PREFIX", "nebula_pairs")
